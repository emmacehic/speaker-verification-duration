...
def create_score_distribution_graphs(results_dataframe):
    ...
    for duration in TEST_DURATIONS:
        ...
        same_median = same_scores.median()
        different_median = different_scores.median()
        ...
        plt.axvline(
            same_median,
            linestyle="--",
            label=f"Same-speaker median: {same_median:.3f}",
        )

        plt.axvline(
            different_median,
            linestyle="--",
            label=f"Different-speaker median: {different_median:.3f}",
        )

        plt.xlabel("Cosine similarity")
        plt.ylabel("Number of trials")
        plt.title(
            f"Score Distributions for {duration}-Second Recordings"
        )
