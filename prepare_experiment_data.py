    ...
    with TRIAL_FILE.open("r", encoding="utf-8") as file:
        ...
        for line_number, line in enumerate(file, start=1):
            ...
            if len(parts) != 3:
                ...
                print(f"Skipping line {line_number}: " "expected 3 columns.")
...
def select_usable_trials(trials: list[dict],) -> list[dict]:
    ...
    for trial in tqdm(trials, desc="Checking official trials",):
        ...
        try:
            second_duration = duration_seconds(second_path)
        ...
        usable_trial["second_duration"] = (second_duration)
...
def balanced_limit(trials: list[dict], limit: int | None,) -> list[dict]:
    ...
    selected = (same_speaker[:same_needed] + different_speakers[:different_needed])
...
def prepare_audio(trials: list[dict]) -> None:
    ...
    for trial_number, trial in enumerate(
        ...
        for duration in DURATIONS:
            ...
            if second_audio is None:
                ...
                second_audio = load_mono_16k(trial["second_path"])
            ...
            sample_count = (duration * TARGET_SAMPLE_RATE)
            ...
            save_audio(test_output,clip,)

            # Updating the cache.
            prepared_tests[duration].add(trial["second_file"])
        ...
        manifest_rows.append(
            {
                ...
                "label_meaning": ("same speaker"
                    if trial["label"] == 1
                    else "different speakers"
                    ),

                "original_reference": (trial["first_file"]),

                "original_test": (trial["second_file"]),
               
                "original_test_duration_seconds": round(trial["second_duration"],4,),
                
                "prepared_reference": str(reference_output.relative_to(PROJECT_ROOT)),

                "prepared_test_3s": str(test_outputs[3].relative_to(PROJECT_ROOT)),
                
                "prepared_test_5s": str(test_outputs[5].relative_to(PROJECT_ROOT)),

                "prepared_test_10s": str(test_outputs[10].relative_to(PROJECT_ROOT)),
            }
        )
    ...
    with SELECTED_TRIALS_FILE.open("w", encoding="utf-8",) as file:
...
def main() -> None:
    ...
    if not TRIAL_FILE.is_file():
        ...
        raise SystemExit("Verification file not found.\n" f"Expected: {TRIAL_FILE}")
    ...
    selected_trials = balanced_limit(usable_trials,args.limit,)
