...
def create_score_distribution_graphs(results_dataframe):
    """
    Create one score-distribution histogram
    for each recording duration.

    Each graph shows the EER threshold for that duration.
    """

    # Labels used in the titles of the three graphs.
    plot_titles = {
    ...
    # Use the same bins for all three graphs.
    # This makes the distributions easier to compare.
    ...
    for duration in TEST_DURATIONS:
        ...
        # Get the labels and scores needed to calculate the EER threshold.
        ...
        # Calculate one EER threshold for the current recording duration.
        ...
        # Draw one dashed line showing the EER threshold.
        plt.axvline(
            eer_threshold,
            color="tab:blue",
            linestyle="--",
        ...
        # Show the threshold value next to the dashed line.
        current_axis = plt.gca()
        maximum_y_value = current_axis.get_ylim()[1]

        plt.text(
            eer_threshold + 0.01,
            maximum_y_value * 0.92,
            f"{eer_threshold:.3f}",
            rotation=90,
        ...
        plt.title(plot_titles[duration])
