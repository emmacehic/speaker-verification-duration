...
PREPARED_AUDIO_FOLDER = Path("data/voxceleb1/prepared_audio")

REFERENCE_FOLDER = (PREPARED_AUDIO_FOLDER / "reference")
...
TRIAL_FILE = (PREPARED_AUDIO_FOLDER / "selected_trials.txt")
            ...
            trials.append(
                {
                    ...
                    "first_path": (REFERENCE_FOLDER / first_file),

                    # Prepared test recording for each duration.
                    "test_paths": {duration: (TEST_FOLDERS[duration] / second_file)
...
def select_usable_trials(all_trials):
    ...
    for trial in tqdm(all_trials, desc="Checking prepared files",):
...
def calculate_eer(labels, scores):
    ...
    differences = np.abs(false_positive_rates - false_negative_rates)
    ...
    eer = (false_positive_rates[closest_index] + false_negative_rates[closest_index]) / 2
    ...
    for duration in TEST_DURATIONS:
        ...
        for trial_number, trial in enumerate(
            ...
            if first_key not in first_embedding_cache:
                ...
                first_embedding_cache[first_key] = (extract_embedding(model, first_waveform,))
            ...
            if second_key not in test_embedding_cache:
                ...
                test_embedding_cache[second_key] = (extract_embedding(model,second_waveform,))

            second_embedding = (test_embedding_cache[second_key])

            # This score is the model output used to decide whether the speakers match.
            similarity_score = calculate_cosine_similarity(first_embedding,second_embedding,)
...
def create_summary(results_dataframe):
    ...
    for duration in TEST_DURATIONS:
        ...
        duration_results = results_dataframe[results_dataframe["duration_seconds"] == duration]

        # NumPy arrays are used because it needs array-like values.
        labels = duration_results["label"].to_numpy()

        scores = duration_results["cosine_similarity" ].to_numpy()
        ...
        mean_same_score = same_speaker_results["cosine_similarity"].mean()

        # The same calculation is repeated for the different-speaker group.
        mean_different_score = different_speaker_results["cosine_similarity"].mean()

        # Medians are included because they are less sensitive to unusual score values.
        median_same_score = same_speaker_results["cosine_similarity"].median()

        median_different_score = different_speaker_results["cosine_similarity"].median()
...
def create_eer_graph(summary_dataframe):
    ...
    plt.savefig(RESULTS_FOLDER / "eer_by_duration.png", dpi=300,)
...
def create_average_score_graph(summary_dataframe):
    ...
    durations = summary_dataframe["duration_seconds"].to_numpy()

    same_scores = summary_dataframe["mean_same_speaker_score"].to_numpy()

    different_scores = summary_dataframe["mean_different_speaker_score"].to_numpy()
    ...
    plt.savefig(RESULTS_FOLDER / "average_scores.png", dpi=300,)
...
def create_median_score_graph(summary_dataframe):
    ...
    same_scores = summary_dataframe["median_same_speaker_score"].to_numpy()

    different_scores = summary_dataframe["median_different_speaker_score"].to_numpy()
    ...
    plt.savefig(RESULTS_FOLDER / "median_scores.png", dpi=300,)
...
def main():
    ...
    if not PREPARED_AUDIO_FOLDER.exists():
        ...
        print("Prepared audio folder not found: " f"{PREPARED_AUDIO_FOLDER}")
    ...
    if not TRIAL_FILE.exists():
        ...
        print(f"Trial file not found: " f"{TRIAL_FILE}")
    ...
    results_dataframe = run_experiment(model, usable_trials,)
    ...
    create_score_distribution_graphs(results_dataframe)

    print(f"Results saved in: " f"{RESULTS_FOLDER.resolve()}")
