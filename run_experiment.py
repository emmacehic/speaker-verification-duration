...
The second recording is cropped to the selected duration.
...
def read_trials():
    ...
    with TRIAL_FILE.open("r", encoding="utf-8") as file:
        for line_number, line in enumerate(file, start=1):
            ...
            # Both the original names and full paths are saved for later stages.
            trials.append(
                {
                    "label": label,
...
def get_audio_duration(audio_path):
    ...
    # File metadata is enough here, so the complete waveform does not need to be loaded.
...
def select_usable_trials(all_trials):
    ...
    """
    Keep trials whose files exist and whose second file
    is at least 10 seconds long.
    """
    ...
    # Every pair is checked before the more expensive model inference begins.
    for trial in tqdm(all_trials, desc="Checking trial files"):
        first_path = trial["first_path"]
        second_path = trial["second_path"]

        # A verification score cannot be produced unless both recordings are available.
        if not first_path.exists() or not second_path.exists():
            missing_files += 1
            continue

        # Existing files can still be corrupted, so duration reading is protected here.
        try:
        ...
        # Ten seconds is the longest condition, which makes it the minimum usable length.
    ...
    print(
        ...
        f"{short_files} under 10 seconds."
    )
...
def crop_audio(waveform, duration_seconds):
    ...
    """Keep the first requested number of seconds."""

    # Seconds are converted to a sample index.
    number_of_samples = duration_seconds * SAMPLE_RATE
    return waveform[:number_of_samples]
...
def run_experiment(model, trials):
    ...
    for duration in TEST_DURATIONS:
        for trial_number, trial in enumerate(
            ...
            # Load each second recording once, then crop it for each test duration.
            if second_key not in test_audio_cache:
                test_audio_cache[second_key] = load_audio(
                    second_path
                )

            second_waveform = test_audio_cache[second_key]

            # Only the second recording is shortened, as said in the experiment design.
            cropped_waveform = crop_audio(
                second_waveform,
                duration,
            )

            # A fresh embedding is required because each duration contains different speech.
...
def main():
    ...
    # Stopping early if required data is missing.
    ...
    if not TRIAL_FILE.exists():
        print(
            f"Trial file not found: {TRIAL_FILE}"
        )
        return
    ...
    if arguments.limit is not None:
        ...
        print(
            f"Using {len(usable_trials)} trials."
        )
